...
class Dataset( torch.utils.data.Dataset):
    ...
    def __getitem__( self , idx ):
        #filename processing
        blurry_img_name = self.img_list[idx].split(' ')[-1]
        clear_img_name = self.img_list[idx].split(' ')[-2]

        blurry_img = Image.open( './training_set/'+blurry_img_name )
        clear_img = Image.open( './training_set/'+clear_img_name )
...
class TestDataset( torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        blurry_img = Image.open('./testing_set/' + blurry_img_name)
        clear_img = Image.open('./testing_set/' + clear_img_name)
        assert blurry_img.size == clear_img.size
        crop_left = int(np.floor(np.random.uniform(0, blurry_img.size[0] - self.crop_size[0] + 1)))
        crop_top = int(np.floor(np.random.uniform(0, blurry_img.size[1] - self.crop_size[1] + 1)))
        crop_location = (crop_left, crop_top, crop_left + self.crop_size[0], crop_top + self.crop_size[1])
